...
const StudentDashboard: React.FC<Props> = ({ assignments, submissions, onNewSubmission }) => {
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-2xl font-bold text-slate-800">Available Assignments</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {assignments.map(a => {
          const mySub = submissions.find(s => s.assignmentId === a.id);
          return (
            <div key={a.id} className="bg-white border rounded-2xl overflow-hidden shadow-sm flex flex-col">
              <div className="p-6 flex-1">
                ...
                <p className="text-slate-600 mb-6">{a.question}</p>
                {mySub && (
                  <div className="bg-indigo-50 p-4 rounded-xl">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-indigo-800 font-bold">Your Grade:</span>
                      <span className="text-2xl font-black text-indigo-600">{mySub.score} / {mySub.maxScore}</span>
                    </div>
                    <p className="text-sm text-indigo-700 italic">" {mySub.feedback} "</p>
                  </div>
                )}
              </div>
              {!mySub && (
        ...
        {assignments.length === 0 && (
          <div className="col-span-full py-20 text-center border-2 border-dashed rounded-2xl bg-white">
            <p className="text-slate-400">No assignments posted by the teacher yet.</p>
          </div>
        )}
      </div>
      ...
      {selectedAssignment && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-8 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h3 className="text-2xl font-bold mb-2">Submit for {selectedAssignment.title}</h3>
            <p className="text-slate-500 mb-6">Upload a photo of your handwritten work or a screenshot.</p>
            ...
            <div className="space-y-4">
              ...
              <div className="p-8 border-2 border-dashed border-slate-200 rounded-2xl text-center bg-slate-50">
                ...
                {!answerImage ? (
                  <label htmlFor="student-answer" className="cursor-pointer">
                    <div className="text-indigo-600 font-bold mb-1">Click to upload photo</div>
                    <div className="text-xs text-slate-400 uppercase">PNG, JPG up to 10MB</div>
                  </label>
                ) : (
                  <div className="relative group">
                    <img src={answerImage} className="max-h-48 mx-auto rounded-lg shadow" alt="My work" />
                    <button 
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 py-3 border rounded-xl font-bold text-slate-600 hover:bg-slate-50 disabled:opacity-50"
                >
                  Cancel
                </button>
                <button 
                  ...
                  className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isSubmitting ? (
                    <>
